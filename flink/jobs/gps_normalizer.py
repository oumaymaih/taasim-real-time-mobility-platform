...
def resolve_zones_geojson_path() -> Path:
    env_path = os.getenv("CASA_ZONES_GEOJSON")
    if env_path:
        return Path(env_path)

    candidate_paths = [
        Path("data/casablanca_arrondissements.geojson"),
        Path("datasets/Arrondissements.geojson"),
        Path("datasets/Arrondissements (1).geojson"),
        Path("Arrondissements.geojson"),
        Path("Arrondissements (1).geojson"),
    ]
    ...
    # Default path shown in the error message if no file is found.
    return candidate_paths[0]


ZONES_GEOJSON_PATH = resolve_zones_geojson_path()
...
ASSIGN_NEAREST_ZONE_IF_OUTSIDE = (
    os.getenv("ASSIGN_NEAREST_ZONE_IF_OUTSIDE", "false").lower() == "true"
)
...
logger.info(f"GeoJSON zones path = {ZONES_GEOJSON_PATH}")
...
def clean_arrondissement_name(value: Any) -> str:
    """Return a clean French arrondissement name from the GeoJSON label.

    The GeoJSON labels can contain Arabic text and parenthesized parts such as:
    "arrondissement d'Aîn-Chock (عين الشق (المقاطعة".
    For Kafka / logs / Grafana, we keep the readable French name only: "Aîn-Chock".
    """
    if value is None:
        return "unknown"
    ...
    # Remove Arabic characters that are sometimes kept without parentheses.
    name = re.sub(r"[\u0600-\u06FF]+.*$", "", name).strip()

    # Remove the French administrative prefix.
    prefixes = [
        "arrondissement d'",
        "arrondissement de ",
        "arrondissement des ",
    ...
    # Normalize common spacing problems.
    name = re.sub(r"\s+", " ", name).strip(" -_'")

    return name or "unknown"


def normalize_ignore_name(value: Any) -> str:
    return clean_arrondissement_name(value).strip().lower()
...
class ArrondissementZoneLookup:
    ...
    def load(self):
        ...
        for feature in features:
            ...
            name = clean_arrondissement_name(raw_name)

            if (
                str(raw_name).strip().lower() in IGNORE_ARRONDISSEMENTS
                or normalize_ignore_name(raw_name) in IGNORE_ARRONDISSEMENTS
                or name.strip().lower() in IGNORE_ARRONDISSEMENTS
            ):
                logger.info(f"Arrondissement ignoré depuis GeoJSON: {raw_name}")
                continue
